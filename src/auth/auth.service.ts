...
import {
  ...
  ICreateUser,
  IResultUser,
} from 'src/users/interface/users.interface';
...
export class AuthService {
  ...
  async login(account: any, response: Response): Promise<any> {
    ...
    const maxAge = parseInt(
      this.configService.get<string>('JWT_REFRESH_EXPIRE') ?? '86400000',
      10,
    );
  ...
  async updateAccount(
    ...
    const user = await this.usersService.updateAcount(id, updateData);
    if (!user) {
